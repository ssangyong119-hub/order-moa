-- 오더모아 단위 8a — 회사 생성 + owner 멤버 부트스트랩 RPC
...
create or replace function public.create_company_with_owner(p_name text, p_business_number text default null)
returns uuid
language plpgsql
security definer
...
begin
  ...
  insert into public.companies (name, business_number, owner_user_id)
