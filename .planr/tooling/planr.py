...
def acquire_status_file_lock(handle: Any) -> None:
...
def release_status_file_lock(handle: Any) -> None:
    if fcntl is not None:
...
def status_write_lock(root: Path) -> Iterator[None]:
    ...
    with lock_path.open("a+", encoding="utf-8") as handle:
        ...
        while True:
            try:
                acquire_status_file_lock(handle)
                break
        ...
        try:
            ...
            release_status_file_lock(handle)
