...
> 제품 메모(2026-06-30, 2026-07-07 갱신): F11은 단순 조회표가 아니라 **매입처 발주 시간 단축을 위한 빌드업**이다. 1차는 거래처 여러 곳의 주문을 품목별로 합산해 `콩나물 7박스, 양파 3망`처럼 확인하고, 품목별 기본 매입처 기준으로 **매입처별 발주 문장**을 복사할 수 있게 한다. 매입처별 실제 원가·발주 기록 저장·재고 반영은 정확 마진과 함께 2차로 분리한다.
>
> **갱신(2026-07-07, 헌장 정렬)**: 위 메모의 "2차 확장"은 앞당겨졌다. 품목별 기본 매입처 표시 + 매입처별 발주 문장 분리/복사는 **1차에서 구현 완료**(합산표 화면, 체크한 품목만 문장에 포함). 매입처의 DB 영구 저장(`ordermoa_suppliers`)은 **1차 보강(8c 전 권장, Codex 승인 후 마이그레이션)**. 매입처별 원가·발주 기록 저장·재고 반영은 여전히 2차.
...
단가 변경 이력 · 매입처별 발주 기록 저장/전송(발주 문장 생성·복사는 1차 구현됨) · **정확한 회계 마진**(매입처별 원가·매입이력·재고 평가·세무 반영, 1차 예상 참고 마진과 구분) · 미수금/입금 본격 관리 · 오래된 미수금 알림 · 거래처별 월 정산표 · 직원 계정/권한 UI · 세금계산서 발행 금액 부족/초과 표시 · 거래처 주문 링크 · OCR · 외부 ERP 연동.
