...
- 검증 최신값(2026-07-08): root `npm test` 5/5 · web `npm test` **103/103** · `npm run build` 성공 · `npm audit --audit-level=low` 0건 · `git diff --check` 통과.

## 다음에 이어서 할 일 (우선순위)
1. **W12 = 거래처별 월 합계** — 저장 주문 기반으로 거래처별 월 합계를 조회한다. 세금계산서 직접 발행이 아니라 월말 확인용 근거 숫자까지만.
2. **잔여 보강** — 실제 인쇄물 육안 확인, 품목 15개 이상 다중 페이지 정돈, 명세서 번호 자동 채번은 별도 승인 후.
