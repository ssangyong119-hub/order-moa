...
- **W21 메모(2026-07-09, 카탈로그 import 미리보기/검수 + DB 반영)**: W20 초안(1590품목)을 앱에서 업로드·검수→선택분만 기준정보로 반영하는 흐름. `catalog-import.ts`(순수 로직+test) · `catalog-import-view.tsx`(업로드·요약·필터·50/페이지·행편집·중복명 경고·기존일치). 데이터 뷰를 "데이터 내보내기/가져오기"로 확장(새 메뉴 없음). 매핑: **입고단가→basePurchasePrice, 출고단가는 저장 안 함**(기본 판매단가는 Phase 3), category→products.category(0007), code→source_code(0008). DB 모드는 `applyCatalogImportToDb`가 200건 배치 insert/update, source_code 멱등 재분류, 별칭 upsert 충돌 무시 후 products 재조회. 데모 모드는 메모리 반영. **0007·0008 Supabase 적용 완료**(사용자 SQL Editor `Success. No rows returned`). 대량 반영은 공유 쿼터 부담 때문에 10~20건 소량 DB smoke 후 진행 권장. 상세: `docs/agent-worklog.md`(2026-07-09 W21).
...
- **W21-C 메모(2026-07-09, DB 소량 smoke 준비 + 전략 점검)**: W21-A/B는 커밋 `66d1af2`로 반영됨. 실 DB smoke는 **로그인 인증 필요 → Claude(헤드리스) 실행 불가**, 사용자 로그인 세션에서 실행한다. Claude 수행분: 자동 검증 재확인(위 최신값 그대로 통과) + **코드 정적 검증으로 "중요 확인" 항목 코드 레벨 보장**(import 경로에 `repSalePrice`/`sale_price`/`customer_prices` 쓰기 0건, `base_purchase_price`=입고단가만, `order_items` 무관, 멱등·별칭충돌무시 유닛테스트) + **깨끗한 신규 15건 선정**(코드 `10006,10013,10014,10017,10020,10021,10024,10026,10028,10036,10037,10038,10057,10063,10068`; needsReview 2=`홍땡초`,`깨순`). 실행 절차·전략은 `docs/agent-worklog.md`(2026-07-09 W21-C). **판단: 15건 smoke로 경로만 검증 → 전체 1590건은 전용 Supabase 분리 후 반영**(공유 쿼터 리스크>이득). 진행판 `progress-data.json`은 실측 미실행이라 미변경.

## 다음에 이어서 할 일 (우선순위)
0. **W18 Phase 1 UX** — 주문일(KST) 표시/수정 · 파싱 화면 단가 Enter이동+변경단가 전체저장 · 주문목록 날짜조회. 마이그레이션 없음, 데모 실측 완료(합산표/월합계 무영향 확인함).
1. **W21 DB 소량 smoke (W21-C, 사용자 실행 대기)** — 0007+0008 적용 완료. Claude 준비 완료(자동 검증·코드 정적 검증·15건 선정·절차). **남은 것은 사용자가 로그인 세션에서 15건 반영 후 수치 확인**: 신규/수정/별칭/실패 배치, F5 유지, 같은 파일 재반영 시 신규 0·수정 15(멱등), `customer_prices` 무변화, 파싱→확정→합산표→명세서 회귀, 콘솔 0. 절차·선정코드는 `agent-worklog.md`(2026-07-09 W21-C). **전체 1590건 반영은 지금 하지 말 것 — 전용 Supabase 분리 후**(공유 쿼터 리스크).
