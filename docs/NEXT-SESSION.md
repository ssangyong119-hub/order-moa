...
- 검증 최신값(2026-07-09, W19): root `npm test` 5/5 · web `npm test` **133/133** · `npm run build` 성공 · `npm audit --audit-level=low` 0건 · `git diff --check` 통과.
...
2. **Phase 3 — 다단위 + 기본단가/거래처 예외(product_units)** — 스키마 설계 필요, 별도 Phase. **아직 미착수(백로그).**
