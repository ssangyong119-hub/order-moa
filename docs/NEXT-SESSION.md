...
- **✅ 완료**: W01(헌장) · **W02 거래처 · W03 품목·별칭 · W04 단가 · W07 매입처(연락처/주소 포함) = 기준정보 4종 전부** · W05·W06(매입처 DB화) · W08(주문 저장 실측) · **W09(raw_text 저장/삭제)** · **W10(저장 주문 기반 합산표)** · W17(합산표 개선 팩).
...
- 검증 최신값(2026-07-08): root `npm test` 5/5 · web `npm test` **102/102** · `npm run build` 성공 · `npm audit --audit-level=low` 0건 · `git diff --check` 통과.

## 다음에 이어서 할 일 (우선순위)
1. **[D] W11 = 저장 주문 기반 거래명세서 재출력** — 주문 목록에서 과거 주문을 다시 열어 인쇄할 때 확정 당시 단가 스냅샷임을 화면/검증으로 더 명확히 한다.
2. **[E] F7 거래명세서 하단 보정** — 인수자 서명란/빈행/합계 문구. 짧은 보정, 중간에 끼워도 됨. (현재 명세서에 인수자란 없음 — 이번 세션에서 미구현 확인됨)
