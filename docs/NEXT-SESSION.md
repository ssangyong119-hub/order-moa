...
3. **W23-R5a ✅ 구현·데모 smoke 완료(2026-07-11, Claude/Codex)** — 마이그레이션 없는 운영 UX 4종: ①세 금액 입력(파싱·가격마감·단가관리) 전체선택(focus)·정규화(blur, 05→5)·Enter 이동(파싱·가격마감만; 단가관리는 Enter=저장 유지) ②단가 관리에 거래처별/기본 출고단가/적용 단가 3열 + "기본 단가 적용 중" 상태(노란 미등록 대신) ③매입처 발주 문장에 발주일 첫 줄 + 매입처별 편집 가능 textarea + `문장 다시 만들기`(편집분 있으면 확인)·전체 복사에 편집 반영 ④합산표 행에서 이번 발주만 매입처 override(productId+unit, products 무변경, 보관 매입처 제외). 거래처·날짜 필터가 바뀌면 임시 재지정·문장 편집본도 비워 다른 발주에 섞이지 않는다. 신규 `web/src/lib/price-input.ts`, `aggregate.ts`(date·override·aggregateRowKey), `price-store.ts`(effective), page/price-management/globals. web **186/186**, 콘솔 0. **R3 상태·스냅샷·source_code 무변경.** 기준/지시: `docs/superpowers/specs/2026-07-11-order-moa-operations-ux-tax-design.md`, `docs/task-prompt-W23-R5a-operational-ux-pack.md`.
4. **W23-R4 — 대시보드 오늘 업무 큐 (다음 후보)** — R5b(정정)와 독립. R6~R8은 `docs/order-moa-expanded-roadmap.md` W23 단계표.
