...
2. **W23-R3 — 가격 마감 (다음 구현)** — 가격 대기 주문에서 품목별 매입가 1회 입력 → 라인별 판매가 제안(거래처 단가 > 기본 출고단가)·참고 마진 → 수정 → **최종 확정**(`closeOrderPrices`: items unit_price UPDATE → orders.status='confirmed' 순서 — 결정 기록 §3.3). `이번 주문만`/`거래처 기본 단가에도 저장`/`품목 기본 매입가에도 저장` 전부 명시 선택, 마진 자동화·자동 저장 금지. 이후 R4(대시보드 오늘 업무 큐) → R5(수정 UX+모바일 결함 3종) → R6~R8은 `docs/order-moa-expanded-roadmap.md` W23 단계표.
