...
## 다음에 이어서 할 일 (우선순위)
0. **W23-R1 — 상태·저장 모델 확정 + 마이그레이션 설계 (Codex 승인 게이트)** — **작업 프롬프트: `docs/task-prompt-W23-R1-state-model-design.md` (전문을 Claude에 전달)**. 재설계 기준 §11의 권장안(orders 단일 소스 유지) 검증: 기존 `draft` 재사용 vs 신규 상태, `unit_price` nullable vs 별도 가격 상태/시각을 비교한다. nullable 후보는 generated `amount`·기존 조회(합산·명세서·월합계)와 안전하게 공존하는지 실측 설계 포함. 기존 확정 주문 backfill 0건 목표.
