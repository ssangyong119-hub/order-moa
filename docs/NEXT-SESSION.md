...
- 검증 최신값(2026-07-08): root `npm test` 5/5 · web `npm test` **111/111** · `npm run build` 성공 · `npm audit --audit-level=low` 0건 · `git diff --check` 통과.

## 다음에 이어서 할 일 (우선순위)
1. **[위험] 전용 Supabase 분리 논의** — 공유 쿼터 경고 상태. 이제 CSV 백업 수단이 생겨 분리 실행 리스크가 낮아짐 → 검토 적기.
2. **엑셀 가져오기(import)** — W15 나머지 절반(초기 세팅용, 후순위). 백업에 archived 포함도 여기서.
3. **잔여 보강** — 인쇄 실물 육안 확인(A4 1장), 품목 15개↑ 다중 페이지, 명세서 번호 자동 채번(별도 승인).
4. **1.5차/2차** — W14(명세서 양식 심화), 미수금·세금계산서 근거(2차)는 헌장 순서대로.
