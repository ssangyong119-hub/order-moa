...
1. **W21 DB 소량 smoke** — 0007+0008 적용 완료 상태. 로그인 모드에서 `docs/order-moa-catalog-real-draft.json` 또는 소량 JSON으로 10~20건만 선택 반영 → 품목·별칭 관리 표시/F5 유지/같은 파일 재반영 시 신규 중복 없음 확인. 공유 Supabase 쿼터 부담 때문에 전체 1590건 반영은 전용 프로젝트 논의 후 권장.
