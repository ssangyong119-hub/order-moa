...
- **W19 메모(2026-07-09, Phase 2 카테고리)**: 품목 카테고리 6종(농산물/공산품/냉식/육류/수산/기타) 도입. **컬럼 1개**(`ordermoa_products.category text NOT NULL DEFAULT '기타' CHECK 6종`) — 6종 고정이라 별도 테이블 없음. **마이그레이션 0007 작성 완료(적용 대기 — Codex 승인 후 SQL Editor)**. 0007 미적용 DB에서도 안 깨지게 읽기/쓰기 폴백(`isMissingCategoryColumn`), 즉석 등록은 DB DEFAULT로 처리. 품목·별칭 관리 화면에 카테고리 select·목록 표시·필터 추가. **order_items·합산표·명세서·월합계·파서 전부 무변경**(카테고리=품목 마스터 속성, 스냅샷 무관). 적용 가이드: `docs/guide-apply-0007-product-category.md`. 상세: `docs/agent-worklog.md`(2026-07-09 W19). Phase 3(다단위·기본/예외 단가)는 백로그 유지·미구현.
...
- 검증 최신값(2026-07-09, W20): root `npm test` 5/5 · web `npm test` **133/133**(앱 코드 무변경) · `npm run build` 성공 · `npm audit --audit-level=low` 0건 · `git diff --check` 통과.

## 다음에 이어서 할 일 (우선순위)
0. **W18 Phase 1 UX** — 주문일(KST) 표시/수정 · 파싱 화면 단가 Enter이동+변경단가 전체저장 · 주문목록 날짜조회. 마이그레이션 없음, 데모 실측 완료(합산표/월합계 무영향 확인함).
1. **Phase 2 — 카테고리 6종 = 구현 완료(2026-07-09, W19), 0007 적용 대기.** Codex 승인 후 `docs/guide-apply-0007-product-category.md` 순서로 SQL Editor 적용 → DB 실측. 컬럼 1개·RLS 상속·idempotent(적용 전 코드는 폴백으로 안 깨짐).
2. **W21 — 실제 엑셀 기반 품목·단위·단가 import (미리보기/검수 UI + DB 반영)** — W20 초안(`docs/order-moa-catalog-real-draft.json`, 1590품목)을 입력으로. (a) 미리보기·검수 화면(카테고리/단위/needsReview 편집) → (b) 승인분만 DB 반영(품목·단위·단가). **DB 반영은 Codex 승인 게이트.** 기존 sampleProducts는 데모/테스트 백업으로 유지(실데이터 우선). 기존 "엑셀 가져오기(W15 나머지 절반)"를 이 흐름으로 통합.
3. **Phase 3 — 다단위 + 기본단가/거래처 예외(product_units)** — 스키마 설계 필요, 별도 Phase. **W20 실데이터가 근거 뒷받침**(같은 품목명 다규격/다단위 210그룹). 아직 미착수(백로그).
4. **[위험] 전용 Supabase 분리 논의** — 공유 쿼터 경고. CSV 백업 수단 있어 분리 리스크 낮아짐 → 적기.
...
### Phase 2에서 구현 완료 (W19, 2026-07-09 · 0007 적용 대기)
- **품목 카테고리 6종** (농산물/공산품/냉식/육류/수산/기타) — 컬럼 1개(`category`, 6종 CHECK, DEFAULT '기타'). 품목·별칭 관리에 select·목록·필터. 0007 마이그레이션 작성 완료(적용은 Codex 승인 후 SQL Editor). 스냅샷·집계 무영향.
