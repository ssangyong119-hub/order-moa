...
- 검증 최신값(2026-07-08): root `npm test` 5/5 · web `npm test` **100/100** · `npm run build` 성공 · `npm audit --audit-level=low` 0건 · `git diff --check` 통과.

## 다음에 이어서 할 일 (우선순위)
1. **[B/C/D] W09~W11 = 8c** — raw_text 저장/삭제(W09) → 저장 주문 기반 합산표 재조회(W10) → 저장 주문 기반 명세서 재출력(W11). 합산 계산 함수는 이미 완성, DB 조회 결과만 연결.
