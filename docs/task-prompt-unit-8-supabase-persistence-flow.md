...
- 매입처 테이블·품목별 기본 매입처·매입처별 발주 저장/전송 — **2차**
...
- 2차 "품목별 기본 매입처"는 **현재 스키마를 막지 않는다**: `suppliers` 테이블 + `products.default_supplier_id`(nullable FK) 추가는 순수 additive. 합산표를 매입처별로 그룹핑하는 확장도 기존 order_items 합산 위에 얹는 형태.
